...
// 알반 팝업 창과 확인 팝업 창을 관리하는 DialogControllerAlert, DialogControllerConfirm의 부모클래스
public class DialogController : MonoBehaviour 
{
    ...
    public bool Visible
    {
        get
        {
            return window.gameObject.activeSelf; // 해당 오브젝트가 활성화 되어있는지 아닌지를 판단하는 읽기 전용 값 activeSelf
        }
        private set
        {
            window.gameObject.SetActive(value); // Visible의 결과에 따라 활성화를 처리하는 코드 외부 접근 불가
        }
    }
